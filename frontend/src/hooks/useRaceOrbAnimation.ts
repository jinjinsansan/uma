...
export interface RaceOrbAnimationState {
  currentConfidence: ConfidenceLevel;
  isColorChanging: boolean;
  colorChangeProgress: number;
  pulseMode: 'normal' | 'racing' | 'prediction' | 'conditions' | 'result';
  isOrbTopic: boolean;
  orbAnimationKey: number;
  currentRandomColor: typeof RANDOM_COLORS[0] | null;
  isShrinking: boolean;
  isExpanding: boolean;
}

export const useRaceOrbAnimation = (
  ...
  // 予想結果表示時のアニメーション
  useEffect(() => {
    if (isPredictionResult && !isConditionsSelected && !isPredictionAnimationActive.current) {
      // アニメーション開始フラグを設定
