...
# Cloudflare CDN URL
NAR_JOCKEY_URL = "https://pub-059afaafefa84116b57d57e0a72b81bd.r2.dev/nankan_jockey_knowledge_20250907.json"
...
def download_existing_jockey_knowledge():
    ...
    local_file = "/mnt/e/dev/Cusor/chatbot/uma/backend/nankan_jockey_knowledge_20250907.json"
    print(f"📥 既存騎手ナレッジファイルを読み込み中: {local_file}")
    
    try:
        with open(local_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
        # JRA騎手版と同じく、metadata不要の構造
        if isinstance(data, dict) and 'metadata' in data:
            data = data.get('jockeys', {})
        ...
        try:
            response = requests.get(NAR_JOCKEY_URL, timeout=60)
...
def update_jockey_knowledge():
    ...
    print("=" * 80)
    print("🏇 地方競馬版（南関東）騎手ナレッジファイル一括更新")
    print("=" * 80)
