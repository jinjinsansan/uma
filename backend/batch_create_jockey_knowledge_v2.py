...
class JockeyKnowledgeBuilder:
    ...
    def get_jockey_venue_course_results(self, jockey_name):
        ...
        for result in all_results:
            year, date, venue_code, position, entry_count, distance = result
            if not distance:
                continue
    ...
    def get_jockey_track_condition_results(self, jockey_name):
        ...
        for result in all_results:
            year, date, position, entry_count, track_code = result
            if not track_code:
                continue
    ...
    def get_jockey_post_position_results(self, jockey_name):
        ...
        for result in all_results:
            year, date, wakuban, position, entry_count = result
    ...
    def get_jockey_sire_results(self, jockey_name):
        ...
        # kyosoba_master2テーブルから父馬名を取得
        query = """
        SELECT 
            u.KAISAI_NEN, u.KAISAI_GAPPI, k.KETTO_TOROKU_BANGO_CH,
