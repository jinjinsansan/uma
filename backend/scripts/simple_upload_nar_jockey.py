...
def sign_request(method, url, headers, payload, access_key, secret_key, region, service='s3'):
    ...
    # 正規リクエストの作成
    canonical_uri = '/' + BUCKET + '/' + 'nankan_jockey_knowledge_20250907.json'
...
def upload_file():
    ...
    local_file = "nankan_jockey_knowledge_20250907.json"
    ...
    # URLとヘッダーの準備
    url = f"{ENDPOINT}/{BUCKET}/nankan_jockey_knowledge_20250907.json"
    headers = {
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            print(f"✅ アップロード成功！")
            print(f"\n🔗 公開URL:")
            print(f"  https://pub-059afaafefa84116b57d57e0a72b81bd.r2.dev/nankan_jockey_knowledge_20250907.json")
            print("\n" + "="*60)
            print("🎉 完了！")
            print("="*60)
            return True
