...
def sign_request(method, url, headers, payload, access_key, secret_key, region, service='s3'):
    ...
    # 正規リクエストの作成
    canonical_uri = '/' + BUCKET + '/' + 'unified_knowledge_20250903.json'
...
def upload_file():
    ...
    # URLとヘッダーの準備
    url = f"{ENDPOINT}/{BUCKET}/unified_knowledge_20250903.json"
    headers = {
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            print(f"✅ アップロード成功！")
            print(f"\n🔗 公開URL:")
            print(f"  https://pub-059afaafefa84116b57d57e0a72b81bd.r2.dev/unified_knowledge_20250903.json")
            print("\n" + "="*60)
            print("🎉 完了！")
            print("="*60)
            return True
