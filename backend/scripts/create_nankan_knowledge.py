...
# -*- coding: utf-8 -*-
"""
南関東競馬統合ナレッジファイル作成スクリプト（正規版）
大井・川崎・船橋・浦和の4競馬場データを統合
JRA版と完全互換性を持つ32フィールド構造
"""
...
def create_nankan_knowledge():
    """南関東競馬統合ナレッジファイル作成"""
    
    print("=" * 80)
    print("🏇 南関東競馬統合ナレッジファイル作成（正規版）")
    print("=" * 80)
    ...
    try:
        ...
        for horse_name in horses_data:
            if horses_data[horse_name] and horses_data[horse_name][0].get('sire'):
                horses_with_sire += 1
            if horses_data[horse_name] and horses_data[horse_name][0].get('broodmare_sire'):
        ...
        output_file = f"nankan_unified_knowledge_{today}.json"
...
def main():
    """メイン処理"""
    print("🏇 南関東競馬ナレッジファイル作成開始")
    print(f"実行時刻: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    result = create_nankan_knowledge()
    
    if result:
        print("\n✅ 正常に完了しました")
        print("次のステップ:")
        print("1. 作成されたファイルをテスト環境で動作確認")
        print("2. 各エンジンでの互換性テスト")
